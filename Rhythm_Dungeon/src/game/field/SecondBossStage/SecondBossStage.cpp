...
// コンストラクタ・デストラクタ
CSecondBossStage::CSecondBossStage() {}
...
// 初期化
void CSecondBossStage::Init() {

	m_vPosition = VGet(0.0f, -170.0f, 600.0f);
	m_iModelHdl = MV1LoadModel("Data/object/SecondBoss/SecondBossStage.mv1");

	//コリジョン情報
	MV1SetPosition(m_iModelHdl, m_vPosition);
	MV1SetScale(m_iModelHdl, VGet(2.5f, 2.5f, 2.5f));
...
VECTOR	CSecondBossStage::HitCheck(VECTOR start, VECTOR end)
{
	VECTOR	out = end;
